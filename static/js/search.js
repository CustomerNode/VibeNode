 ...
 *
 * Distinct from the sidebar #search box (client-side title filter) and from
 * find.js (find-in-current-transcript): this searches the full message
 * content of every session in the active project via GET /api/search.
 *
 * Entry points:
 *   - the magnifier button in the sidebar search row (openDeepSearch()),
 *   - pressing Enter in the sidebar #search box (opens pre-filled — Enter
 *     was previously a no-op there, so this is additive).
 *
...
function openDeepSearch(prefill) {
  ...
  overlay.innerHTML = `
    <div style="background:var(--bg-secondary);border:1px solid var(--border);border-radius:10px;padding:18px 20px;width:640px;max-width:92vw;max-height:74vh;display:flex;flex-direction:column;color:var(--text-primary);font-family:inherit">
      ...
      <p style="margin:0 0 10px;font-size:12px;color:var(--text-muted)">Searches full message content across every session in this project. Use <code>file:name.py</code> to find sessions that edited a file.</p>
...
async function _dsRun(overlay) {
  ...
  status.textContent = sessions.length
    ? `${sessions.length} session${sessions.length === 1 ? '' : 's'} · ${st.messages_indexed || 0} messages indexed · ${st.took_ms || 0} ms`
    : 'No matches';
  results.innerHTML = sessions.map(s => _dsRenderRow(s)).join('');
...
/* Enter in the sidebar #search box opens deep search pre-filled.  Enter was
 * a no-op there before (the box filters on input), so this is additive. */
(function () {
  const el = document.getElementById('search');
  if (!el) return;
  el.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') { e.preventDefault(); openDeepSearch(el.value.trim()); }
  });
})();
