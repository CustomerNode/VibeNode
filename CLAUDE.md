...
2. **`_cli_model_id()` strips display markers everywhere a recorded model becomes a launch arg.** The CLI reports `claude-opus-5[1m]` when 1M context is active and that flows verbatim into `info.model` and the registry. `[1m]` is NOT a valid `--model` id (API 400). Every path that turns a recorded model into `--model` — the wake seed and the `send_message` auto-resume — must go through it. The badge keeps the marker on purpose (honest about context length). Client mirror: `SessionModel._cleanId`.
